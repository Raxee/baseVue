...
import {delSessionById, getContentBySessionId, updateChatLabel} from "@/api/chat";
import {ChatContent} from "@/api/interface";
import {useChat} from "@/views/chat/hooks/useChat";
import {isTourist} from "@/utils/token";
...
async function handleSelect(uuid: string) {
  if (isActive(uuid))
    return
  const res = await getContentBySessionId(uuid);
...
function handleSaveTitle(item: Chat.History, event?: MouseEvent) {
...
function handleDelete(index: number, event?: MouseEvent | TouchEvent) {
  event?.stopPropagation()
  delSessionById(String(chatStore.active));
          ...
          >
            ...
            <div v-if="isActive(item.uuid)" class="absolute z-10 flex visible right-1">
              <template v-if="item.isEdit">
                <button class="p-1" @click="handleSaveTitle(item, $event)">
              ...
              <template v-else>
                ...
                <NPopconfirm placement="bottom" @positive-click="handleDeleteDebounce(index, $event)" v-if="!ifTourist">
                  <template #trigger>
                    <button class="p-1">
